...
class KuramanimeProvider : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        fun queueUrl(url: String?) {
            if (url.isNullOrBlank() || url == "#") return
            val fixedUrl = fixUrl(url).replace("&amp;", "&")
            discoveredUrls.add(fixedUrl)
        }

        document.select("iframe[src], iframe[data-src]").forEach {
            queueUrl(it.attr("src").ifBlank { it.attr("data-src") })
        }
        document.select("video#player > source[src], video#player[src], [data-hls-src]").forEach {
            queueUrl(it.attr("src").ifBlank { it.attr("data-hls-src") })
        }
        document.select("div#animeDownloadLink a[href]").forEach {
            queueUrl(it.attr("href"))
        }

        discoveredUrls.forEach { link ->
            ...
            if (link.contains("pid=") && link.contains("sid=")) {
                ...
                if (pid != null && sid != null) {
                    try {
                        ...
                        if (accessToken != null && gid != null) {
                            ...
                            callback.invoke(
                                newExtractorLink(
                                    source = "Kuramadrive (VIP)",
                                    name = "Kuramadrive (VIP)",
                                    url = realVideoUrl,
                                    type = INFER_TYPE
                                ) {
                ...
                link.contains("p2pstream", true) ||
                link.contains("rpmvip", true) ||
                link.contains("mega", true) ||
                link.contains(".my.id", true) ||
                link.contains("myvidplay", true) ||
                link.contains("dood", true) ||
                ...
                if (link.endsWith(".mp4", true) || link.endsWith(".m3u8", true)) {
                    val serverName = if (link.contains("p2pstream")) "P2P Stream"
                                     else if (link.contains("rpmvip")) "RPM VIP"
                                     else "Kurama CDN"
                        ...
                        runBlocking {
                            callback.invoke(
                                newExtractorLink(
                                    ...
                                    this.referer = extLink.referer ?: "$mainUrl/"
